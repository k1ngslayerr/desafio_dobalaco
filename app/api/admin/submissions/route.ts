...
export async function GET() {
  ...
  const [{ data: subs }, { data: challenges }] = await Promise.all([
    adminClient
      .from("submissions")
      .select("id, photo_url, title, description, status, xp_awarded, created_at, user:users!user_id(id, username, avatar_url), challenge:challenges(id, title, xp_reward), reactions(type)")
